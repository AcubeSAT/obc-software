...
This repository houses the OBC software for both development and EQM. 
...
Either sub-project (obc-dev or obc-eqm) can be built without any access to specialised hardware. We provide build
...
2. Clone the repository and enter the directory:
   ```shell
   git clone git@gitlab.com:acubesat/obc/acubesat-obc-software-merged.git
   cd acubesat-obc-software-merged
   ```
...
5. Download all dependencies and build the project through conan:
   ```shell
   conan install . --output-folder=cmake-build-debug --build="*" -u -pr conan-arm-profile -o project="obc-dev"
   ```
    Pass the string `obc-dev` in the option `project` if you wish to build the development sub-project used on an [evaluation board](https://www.microchip.com/en-us/development-tool/ATSAMV71-XULT), otherwise pass the string `obc-eqm` if you wish to build the sub-project to be used on our EQM.
...
7. Add CMake flags:
   ```shell
   cmake -B cmake-build-debug/build/Debug -DCMAKE_TOOLCHAIN_FILE=cmake-build-debug/build/Debug/generators/conan_toolchain.cmake -DCMAKE_BUILD_TYPE=Debug .
   ```
8. Build the project:
   ```shell
   ./build.sh
   ```
If `./build.sh` doesn't run make sure it is executable:
...
2. Add the following to the CMake Options (File -> Settings -> Build, Execution, Deployment -> CMake -> CMake Options):
   ```
   -DCMAKE_TOOLCHAIN_FILE=cmake-build-debug/build/Debug/generators/conan_toolchain.cmake"
   ```
